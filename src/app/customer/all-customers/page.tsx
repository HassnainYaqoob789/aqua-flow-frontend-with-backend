...
export default function CustomerManagement() {
  ...
  const limit = 10; // Adjust as needed (10, 20, 50, etc.)

  const [activeTab, setActiveTab] = useState("All");
  const [searchTerm, setSearchTerm] = useState("");
  const [openId, setOpenId] = useState<string | null>(null);

  // Fetch customers with current page and limit
  ...
  // Sync data to Zustand store when fetched
  useEffect(() => {
    if (data) {
      setCustomers(data);
    }
  ...
  const counts = useCustomerStore((s) => s.state.counts);

  // Derive pagination info from server response
  ...
  // Optional: Sync page if server returns a different page (e.g. out of bounds)
  useEffect(() => {
  ...
  const stats = [
    { label: "Total Customers", value: counts.totalCustomers.toString() },
    { label: "Active", value: counts.activeCustomers.toString(), color: "text-green-600" },
    { label: "Inactive", value: counts.inactiveCustomers.toString(), color: "text-red-600" },
  ];

  const tabs = [
    `All Customers (${counts.totalCustomers})`,
    `Active (${counts.activeCustomers})`,
    `Inactive (${counts.inactiveCustomers})`,
  ];
  ...
  return (
    <DefaultLayout>
      <Breadcrumb
        ...
        description="Manage and track all water delivery customers"
      />
        ...
        <div className="p-3 sm:p-6">
          ...
          <div className="mb-6 flex flex-col gap-3 sm:gap-4 md:flex-row">
            ...
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="inline-flex items-center gap-2 rounded-lg bg-[#1D4ED8] px-5 py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-[#1E40AF] disabled:opacity-60"
            >
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap rounded-full px-3 py-2 text-xs font-medium transition-colors sm:text-sm ${activeTab === tabName
          ...
          {/* Loading State */}
          {isLoading && (
            <div className="flex items-center justify-center rounded-lg border border-gray-200 bg-white p-16 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          {/* Error State */}
          {isError && (
            <div className="flex items-center justify-center rounded-lg border border-red-200 bg-red-50 p-16 shadow-sm dark:border-red-800 dark:bg-red-900/20">
              <div className="text-center">
          ...
          {!isLoading && !isError && (
            <>
                ...
                <table className="w-full min-w-full">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-700">
                      <th className="px-3 py-3 text-left text-xs font-semibold text-gray-900 dark:text-white sm:px-6 sm:py-4 sm:text-sm">Customer</th>
                      <th className="hidden px-3 py-3 text-left text-xs font-semibold text-gray-900 dark:text-white sm:px-6 sm:py-4 sm:text-sm md:table-cell">Contact & Location</th>
                      <th className="hidden px-3 py-3 text-left text-xs font-semibold text-gray-900 dark:text-white sm:px-6 sm:py-4 sm:text-sm md:table-cell">Zone</th>
                      <th className="hidden px-3 py-3 text-left text-xs font-semibold text-gray-900 dark:text-white sm:px-6 sm:py-4 sm:text-sm lg:table-cell">Security Deposit</th>
                    ...
                    {filteredCustomers.map((customer) => (
                      <tr key={customer.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td className="px-3 py-3 sm:px-6 sm:py-4">
                          <div>
                            <p className="text-xs font-medium text-gray-900 dark:text-white sm:text-sm">{customer.name}</p>
                            <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400 sm:mt-1">{customer.email}</p>
                            <div className="mt-1 space-y-0.5 md:hidden">
                              <p className="text-xs text-gray-600 dark:text-gray-300">📞 {customer.phone}</p>
                              <p className="text-xs text-gray-500 dark:text-gray-400">📍 {customer.address}, {customer.city}</p>
                              <p className="text-xs text-gray-600 dark:text-gray-300">🗺️ {customer.zone?.name || 'N/A'}</p>
                              <p className="text-xs text-gray-600 dark:text-gray-300">Security Deposit: {customer.securityDeposit ?? 0}</p>
                            </div>
                          </div>
                        </td>
                        <td className="hidden md:table-cell px-3 py-3 sm:px-6 sm:py-4">
                          <div className="text-xs sm:text-sm">
                            <p>📞 {customer.phone}</p>
                            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{customer.address}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">{customer.city}, {customer.postalCode}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">{customer.country}</p>
                          </div>
                        </td>
                        <td className="hidden md:table-cell px-3 py-3 sm:px-6 sm:py-4">
                          <span className="inline-block px-3 py-1 text-xs font-semibold text-orange-700 bg-orange-100 dark:text-orange-300 dark:bg-orange-900/30 rounded-md">
                            {customer.zone?.name || 'N/A'}
                          </span>
                        </td>
                        <td className="hidden lg:table-cell px-3 py-3 sm:px-6 sm:py-4">
                          <div className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium text-purple-700 bg-purple-100 dark:text-purple-200 dark:bg-purple-800/40 border border-purple-300 dark:border-purple-700 rounded-sm">
                            <span className="opacity-80">Security Deposit:</span>
                            <span className="font-semibold">{customer.securityDeposit ?? "-"}</span>
                          </div>
                        </td>
                        <td className="px-3 py-3 sm:px-6 sm:py-4">
                          <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(customer.status)}`}>
                            {customer.status}
                          </span>
                        </td>
                        <td className="px-3 py-3 sm:px-6 sm:py-4">
                                ...
                                <div className="absolute right-0 top-full z-50 mt-1 w-40 rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-600 dark:bg-gray-700">
                                  {customer.status === "active" ? (
                                    <button
                                      ...
                                      disabled={isPending}
                                      className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-red-50 dark:text-gray-300 dark:hover:bg-red-900/30"
                                    >
                                      ✗ Deactivate
                                    </button>
                                  ) : (
                                    <button
                                      ...
                                      disabled={isPending}
                                      className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-green-50 dark:text-gray-300 dark:hover:bg-green-900/30"
                                    >
                                      ✓ Activate
                                    </button>
                                  )}
                ...
                {filteredCustomers.length === 0 && (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">
                      {searchTerm || activeTab !== "All" ? "No customers found matching your criteria" : "No customers"}
                    </p>
                  </div>
                )}
              </div>
              ...
              {isFetching && !isLoading && (
                <div className="mt-4 text-center text-sm text-blue-600">Loading next page...</div>
              )}
            </>
          )}
          ...
          {activeTab === "Inactive" && filteredCustomers.length > 0 && (
            <div className="mt-8 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20 sm:p-6">
              <div className="flex gap-3">
                <AlertCircle className="mt-0.5 h-5 w-5 flex-shrink-0 text-red-600 dark:text-red-500" />
                <div className="flex-1">
                  <h3 className="mb-1 font-semibold text-red-900 dark:text-red-100">Inactive Customers Detected</h3>
                  <p className="mb-4 text-sm text-red-800 dark:text-red-200">
                    {filteredCustomers.length} customer{filteredCustomers.length !== 1 ? "s" : ""} currently inactive. Reactivate them to resume orders and services.
                  </p>
