...
export default function AddCustomerModal({ isOpen, onClose }: AddCustomerModalProps) {
  ...
  const { data: zoneData, isLoading, isError, refetch: refetchZones } = useZone();
  ...
  // Effect to set the zone when it becomes available after creation
  useEffect(() => {
  ...
  // Handle zone creation success
  const handleZoneCreated = async (newZoneId: string) => {
    console.log("Zone created with ID:", newZoneId);

    // Store the pending zone ID
    setPendingZoneId(newZoneId);

    // Trigger a refetch to get the updated zones list
    await refetchZones();
  };
                  ...
                  {isLoading ? (
                    ...
                    <>
                      ...
                      <button
                        ...
                        className="flex items-center justify-center rounded-lg bg-[#1C2434] px-4 py-2 text-sm font-semibold text-white shadow-md hover:bg-[#131923] focus:outline-none focus:ring-2 focus:ring-[#1C2434]/30 transition-all duration-200"
                        title="Add New Zone"
                      >
